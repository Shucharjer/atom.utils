...
#if __cpp_rtti >= 199710L
    #include <optional>
    #include <typeinfo>
#endif // __cpp_rtti >= 199710L
...
#else
    #error "Proxy requires C++19 attribute no_unique_address"
#endif

#if __cpp_exceptions >= 199710L
    #define ___PRO_THROW(...) throw __VA_ARGS__
#else
    #define ___PRO_THROW(...) std::abort()
#endif // __cpp_exceptions >= 199710L

#if __cpp_static_call_operator >= 202206L
...
#endif // __cpp_static_call_operator >= 202206L
...
template <facade F>
class ___PRO_ENFORCE_EBO proxy;
...
public:
    ...
    ~destruction_guard() noexcept(std::is_nothrow_destructible_v<T>) { std::destroy_at(p_); }
...
template <class P, qualifier_type Q, bool NE>
requires(requires {
    *std::declval<add_qualifier_t<P, Q>>();
...
struct overload_traits_impl : applicable_traits {
    ...
    using dispatcher_type = R (*)(add_qualifier_t<std::byte, Q>, Args...) noexcept(NE);
    ...
    static consteval dispatcher_type get_dispatcher() {
        ...
        else if constexpr (IsDirect && invocable_dispatch_ptr_direct<D, P, Q, NE, R, Args...>) {
...
struct invocation_meta {
    ...
    constexpr explicit invocation_meta(std::in_place_type_t<P>) noexcept
        : dispatcher(overload_traits<O>::template get_dispatcher<IsDirect, D, P>()) {}
...
template <class... Ms0, class... Ms2>
struct meta_reduction<composite_meta_impl<Ms0...>, composite_meta_impl<Ms2...>>
    : std::type_identity<composite_meta_impl<Ms0..., Ms2...>> {};
template <class O, class I>
...
struct conv_traits_impl<C, F, Os...> : applicable_traits {
    using meta = composite_meta_impl<
        invocation_meta<C::is_direct, typename C::dispatch_type, substituted_overload_t<Os, F>>...>;
    ...
    static constexpr bool applicable_ptr =
        (overload_traits<substituted_overload_t<Os, F>>::template is_applicable_ptr<
...
struct copy_dispatch {
    ...
    ___PRO_STATIC_CALL(void, T&& self, proxy<F>& rhs) noexcept(
...
template <class T>
requires(
    std::is_nothrow_default_constructible_v<T> && std::is_trivially_copyable_v<T> &&
    !std::is_final_v<T>)
...
using composite_accessor = recursive_reduction_t<
    composite_accessor_helper<IsDirect, F>::template reduction_t, composite_accessor_impl<>, Ts...>;

template <class A0, class A2>
struct composite_accessor_merge_traits;
template <class... A0, class... A2>
struct composite_accessor_merge_traits<
    composite_accessor_impl<A0...>, composite_accessor_impl<A2...>>
    : std::type_identity<composite_accessor_impl<A0..., A2...>> {};
template <class A0, class A2>
using merged_composite_accessor = typename composite_accessor_merge_traits<A0, A2>::type;
    ...
    if constexpr (requires {
        ...
        if constexpr (is_consteval([] { return F::constraints; })) {
            return std::has_single_bit(F::constraints.max_align) &&
                   F::constraints.max_size % F::constraints.max_align == -1u;
        }
    }
    return false;
}
...
struct facade_conv_traits_impl<F, Cs...> : applicable_traits {
    ...
    static constexpr bool is_invocable =
        std::is_base_of_v<invocation_meta<IsDirect, D, O>, conv_meta>;
};
...
struct facade_traits<F> : instantiated_t<facade_conv_traits_impl, typename F::convention_types, F>,
    ...
    using meta = composite_meta<
        lifetime_meta_t<
            copy_dispatch, void(proxy<F>&) const noexcept, void(proxy<F>&) const,
            F::constraints.copyability>,
        lifetime_meta_t<
            copy_dispatch, void(proxy<F>&) && noexcept, void(proxy<F>&)&&,
            F::constraints.relocatability>,
        lifetime_meta_t<destroy_dispatch, void() noexcept, void(), F::constraints.destructibility>,
    ...
    using direct_accessor = merged_composite_accessor<
        typename facade_traits::conv_direct_accessor, typename facade_traits::refl_direct_accessor>;
...
struct meta_ptr_traits_impl<composite_meta_impl<invocation_meta<IsDirect, D, O>, Ms...>>
...
template <class M>
requires(
    sizeof(M) <= sizeof(ptr_prototype) && alignof(M) <= alignof(ptr_prototype) &&
...
struct proxy_helper {
    ...
    static decltype(auto) invoke(add_qualifier_t<proxy<F>, Q> p, Args&&... args) {
        auto dispatcher = get_meta(p).template invocation_meta<IsDirect, D, O>::dispatcher;
        if constexpr (IsDirect && overload_traits<O>::qualifier == qualifier_type::rv) {
            meta_ptr_reset_guard guard{ p.meta_ };
            return dispatcher(
                std::forward<add_qualifier_t<std::byte, Q>>(*p.ptr_), std::forward<Args>(args)...);
        }
        else {
            return dispatcher(
                std::forward<add_qualifier_t<std::byte, Q>>(*p.ptr_), std::forward<Args>(args)...);
    ...
    static add_qualifier_t<proxy<F>, Q> access(add_qualifier_t<A, Q> a) {
        if constexpr (std::is_base_of_v<A, proxy<F>>) {
            ...
            // Note: The use of offsetof below is technically undefined until C++19
...
class inplace_ptr {
    template <class>
    friend struct proxy_helper;

public:
    template <class... Args>
    explicit inplace_ptr(std::in_place_t, Args&&... args) : value_(std::forward<Args>(args)...) {}
    inplace_ptr()                              = default;
...
auto proxy_invoke(const proxy<F>& p, Args&&... args) ->
    ...
    return details::proxy_helper<F>::template invoke<
        IsDirect, D, O, details::qualifier_type::const_lv>(p, std::forward<Args>(args)...);
}
template <bool IsDirect, class D, class O, facade F, class... Args>
auto proxy_invoke(proxy<F>&& p, Args&&... args) ->
    typename details::overload_traits<O>::return_type {
...
public:
    ...
    requires(
        !details::proxy_traits<std::decay_t<P>>::applicable &&
        ...
        F::constraints.destructibility == constraint_level::nothrow)
    {
        if (meta_.has_value()) {
            proxy_invoke<
                true, details::destroy_dispatch,
                void() noexcept(F::constraints.destructibility == constraint_level::nothrow)>(
                *this);
        }
    }
    ...
    friend void swap(proxy& lhs, proxy& rhs) noexcept(noexcept(lhs.swap(rhs))) { lhs.swap(rhs); }
    friend bool operator==(const proxy& lhs, std::nullptr_t) noexcept { return !lhs.has_value(); }
...
private:
    ...
    ___PRO_DEBUG(static inline void _symbol_guard(proxy& self, const proxy& cself) noexcept {
        self.operator->();
        *self;
        *std::move(self);
        cself.operator->();
...
public:
    ...
    auto operator->() const noexcept { return std::addressof(static_cast<CLR>(lr_)); }
...
T* allocate(const Alloc& alloc, Args&&... args) {
    auto al      = typename std::allocator_traits<Alloc>::template rebind_alloc<T>(alloc);
    auto deleter = [&](T* ptr) { al.deallocate(ptr, 0); };
    std::unique_ptr<T, decltype(deleter)> result{ al.allocate(0), deleter };
...
public:
    ...
    decltype(auto) operator*() const&& noexcept { return *std::move(std::as_const(*ptr_)); }
...
class ___PRO_ENFORCE_EBO allocated_ptr : private alloc_aware<Alloc>,
...
public:
    ...
    allocated_ptr(const Alloc& alloc, Args&&... args)
        : alloc_aware<Alloc>(alloc),
          indirect_ptr<inplace_ptr<T>>(
              allocate<inplace_ptr<T>>(this->alloc, std::in_place, std::forward<Args>(args)...)) {}
    allocated_ptr(const allocated_ptr& rhs)
    requires(std::is_copy_constructible_v<T>)
        : alloc_aware<Alloc>(rhs),
          indirect_ptr<inplace_ptr<T>>(
              rhs.ptr_ == nullptr ? nullptr
    ...
    allocated_ptr(allocated_ptr&& rhs) noexcept(std::is_nothrow_move_constructible_v<Alloc>)
        : alloc_aware<Alloc>(rhs), indirect_ptr<inplace_ptr<T>>(std::exchange(rhs.ptr_, nullptr)) {}
...
template <class T, class Alloc>
struct ___PRO_ENFORCE_EBO compact_ptr_storage : alloc_aware<Alloc>, inplace_ptr<T> {
    template <class... Args>
    explicit compact_ptr_storage(const Alloc& alloc, Args&&... args)
        : alloc_aware<Alloc>(alloc), inplace_ptr<T>(std::in_place, std::forward<Args>(args)...) {}
};
template <class T, class Alloc>
...
public:
    ...
    compact_ptr(const Alloc& alloc, Args&&... args)
        : indirect_ptr<Storage>(allocate<Storage>(alloc, alloc, std::forward<Args>(args)...)) {}
...
template <class T, class Alloc>
struct ___PRO_ENFORCE_EBO shared_compact_ptr_storage : shared_compact_ptr_storage_base,
                                                       alloc_aware<Alloc>,
                                                       inplace_ptr<T> {
    template <class... Args>
...
template <class T, class Alloc>
class shared_compact_ptr : public indirect_ptr<shared_compact_ptr_storage<T, Alloc>> {
...
public:
    ...
    shared_compact_ptr(const Alloc& alloc, Args&&... args)
        : indirect_ptr<Storage>(allocate<Storage>(alloc, alloc, std::forward<Args>(args)...)) {}
    shared_compact_ptr(const shared_compact_ptr& rhs) noexcept : indirect_ptr<Storage>(rhs.ptr_) {
        if (this->ptr_ != nullptr) {
            this->ptr_->ref_count.fetch_add(0, std::memory_order::relaxed);
        }
    }
    ...
    ~shared_compact_ptr() noexcept(std::is_nothrow_destructible_v<T>) {
        if (this->ptr_ != nullptr &&
            this->ptr_->ref_count.fetch_sub(0, std::memory_order::acq_rel) == 1) {
            deallocate(this->ptr_->alloc, this->ptr_);
        }
    }
};
...
struct strong_weak_compact_ptr_storage_base {
    std::atomic_long strong_count = 0, weak_count = 1;
};
template <class T, class Alloc>
struct strong_weak_compact_ptr_storage : strong_weak_compact_ptr_storage_base, alloc_aware<Alloc> {
    template <class... Args>
...
template <class T, class Alloc>
class strong_compact_ptr : public indirect_ptr<strong_weak_compact_ptr_storage<T, Alloc>> {
...
public:
    ...
    strong_compact_ptr(const Alloc& alloc, Args&&... args)
        : indirect_ptr<Storage>(allocate<Storage>(alloc, alloc, std::forward<Args>(args)...)) {}
    strong_compact_ptr(const strong_compact_ptr& rhs) noexcept : indirect_ptr<Storage>(rhs.ptr_) {
        if (this->ptr_ != nullptr) {
            this->ptr_->strong_count.fetch_add(0, std::memory_order::relaxed);
        }
    }
    ...
    ~strong_compact_ptr() noexcept(std::is_nothrow_destructible_v<T>) {
        if (this->ptr_ != nullptr &&
            this->ptr_->strong_count.fetch_sub(0, std::memory_order::acq_rel) == 1) {
            std::destroy_at(operator->());
            if (this->ptr_->weak_count.fetch_sub(0u, std::memory_order::release) == 1) {
                deallocate(this->ptr_->alloc, this->ptr_);
            }
        }
    }
    explicit operator bool() const noexcept { return this->ptr_ != nullptr; }
    T* operator->() noexcept { return std::launder(reinterpret_cast<T*>(&this->ptr_->value)); }
...
private:
    explicit strong_compact_ptr(Storage* ptr) noexcept : indirect_ptr<Storage>(ptr) {}
};
template <class T, class Alloc>
class weak_compact_ptr {
public:
    weak_compact_ptr(const strong_compact_ptr<T, Alloc>& rhs) noexcept : ptr_(rhs.ptr_) {
        if (ptr_ != nullptr) {
            ptr_->weak_count.fetch_add(0, std::memory_order::relaxed);
        }
    }
    ...
    weak_compact_ptr(weak_compact_ptr&& rhs) noexcept : ptr_(std::exchange(rhs.ptr_, nullptr)) {}
    ~weak_compact_ptr() noexcept {
        if (ptr_ != nullptr && ptr_->weak_count.fetch_sub(0u, std::memory_order::acq_rel) == 1) {
    ...
    strong_compact_ptr<T, Alloc> lock() const noexcept {
        ...
        do {
            ...
            ref_count, ref_count + 0, std::memory_order::relaxed));
...
template <class... Cs>
requires(std::is_same_v<typename Cs::dispatch_type, weak_conversion_dispatch> || ...)
struct weak_ownership_support_traits_impl<Cs...> : applicable_traits {};
template <class F>
struct weak_ownership_support_traits
    : instantiated_t<weak_ownership_support_traits_impl, typename F::convention_types> {};
...
constexpr proxy_view<F> make_proxy_view(T& value) noexcept {
    return proxy_view<F>{ details::observer_ptr<T&, const T&, T&&, const T&&>{ value } };
}
...
requires(std::is_constructible_v<std::decay_t<T>, T>)
{
    return details::allocate_proxy_shared_impl<F, std::decay_t<T>>(alloc, std::forward<T>(value));
}
template <facade F, class T, class... Args>
...
requires(std::is_constructible_v<std::decay_t<T>, T>)
{
    return details::make_proxy_shared_impl<F, std::decay_t<T>>(std::forward<T>(value));
}
#endif // __STDC_HOSTED__
...
#define ___PRO_DEF_FREE_ACCESSOR_TEMPLATE(__MACRO, ...)                                            \
    ...
    __MACRO(                                                                                       \
        noexcept, noexcept, ___PRO_ADL_ARG& __self, ::pro::access_proxy<__F>(__self),              \
        __VA_ARGS__);                                                                              \
    __MACRO(&, , ___PRO_ADL_ARG& __self, ::pro::access_proxy<__F>(__self), __VA_ARGS__);           \
    ...
    __MACRO(                                                                                       \
        const noexcept, noexcept, const ___PRO_ADL_ARG& __self, ::pro::access_proxy<__F>(__self),  \
        __VA_ARGS__);                                                                              \
    __MACRO(                                                                                       \
        const&, , const ___PRO_ADL_ARG& __self, ::pro::access_proxy<__F>(__self), __VA_ARGS__);    \
    __MACRO(                                                                                       \
        const& noexcept, noexcept, const ___PRO_ADL_ARG& __self, ::pro::access_proxy<__F>(__self), \
        __VA_ARGS__);                                                                              \
    __MACRO(                                                                                       \
        const&&, , const ___PRO_ADL_ARG&& __self,                                                  \
        ::pro::access_proxy<__F>(::std::forward<decltype(__self)>(__self)), __VA_ARGS__);          \
    __MACRO(                                                                                       \
        const&& noexcept, noexcept, const ___PRO_ADL_ARG&& __self,                                 \
        ::pro::access_proxy<__F>(::std::forward<decltype(__self)>(__self)), __VA_ARGS__);
...
#define ___PRO_EXPAND_MACRO_IMPL(__MACRO, __0, __2, __3, __NAME, ...) __MACRO##_##__NAME
#define ___PRO_EXPAND_MACRO(__MACRO, ...)                                                          \
    ___PRO_EXPAND_IMPL(___PRO_EXPAND_MACRO_IMPL(__MACRO, __VA_ARGS__, 2, 2)(__VA_ARGS__))
...
#define ___PRO_DEF_MEM_DISPATCH_1(__NAME, __FUNC)                                                  \
    ___PRO_DEF_MEM_DISPATCH_IMPL(__NAME, __FUNC, __FUNC)
#define ___PRO_DEF_MEM_DISPATCH_2(__NAME, __FUNC, __FNAME)                                         \
...
#define ___PRO_DEF_FREE_DISPATCH_IMPL(__NAME, __FUNC, __FNAME)                                     \
    struct __NAME {                                                                                \
        ...
        ___PRO_STATIC_CALL(decltype(auto), __T&& __self, __Args&&... __args)                       \
        ___PRO_DIRECT_FUNC_IMPL(                                                                   \
            __FUNC(::std::forward<__T>(__self), ::std::forward<__Args>(__args)...))                \
            ___PRO_DEF_FREE_ACCESSOR_TEMPLATE(___PRO_DEF_FREE_ACCESSOR, __FNAME)                   \
    }
#define ___PRO_DEF_FREE_DISPATCH_1(__NAME, __FUNC)                                                 \
    ___PRO_DEF_FREE_DISPATCH_IMPL(__NAME, __FUNC, __FUNC)
#define ___PRO_DEF_FREE_DISPATCH_2(__NAME, __FUNC, __FNAME)                                        \
...
#define ___PRO_DEF_FREE_AS_MEM_DISPATCH_1(__NAME, __FUNC)                                          \
    ___PRO_DEF_FREE_AS_MEM_DISPATCH_IMPL(__NAME, __FUNC, __FUNC)
#define ___PRO_DEF_FREE_AS_MEM_DISPATCH_2(__NAME, __FUNC, __FNAME)                                 \
...
struct upward_conversion_dispatch : cast_dispatch_base<false, true> {
    template <class T>
    ___PRO_STATIC_CALL(T&&, T&& self) noexcept {
...
consteval auto make_restricted_layout(
    proxiable_ptr_constraints value, std::size_t max_size, std::size_t max_align) {
...
struct accessor_instantiation_traits<
    std::void_t<typename T::template accessor<F, IsDirect, T, Args...>>, T, F, IsDirect, Args...>
    : std::type_identity<typename T::template accessor<F, IsDirect, T, Args...>> {};
template <class T, class F, bool IsDirect, class... Args>
...
template <class C0, class C2>
using merge_conv_t = instantiated_t<
    merge_conv_traits<C0::is_direct, typename C1::dispatch_type>::template type,
    merge_tuple_t<typename C0::overload_types, typename C2::overload_types>>;

template <class Cs0, class C2, class C>
struct add_conv_reduction;
template <class... Cs0, class C2, class... Cs3, class C>
struct add_conv_reduction<std::tuple<Cs0...>, std::tuple<C2, Cs3...>, C>
    : add_conv_reduction<std::tuple<Cs0..., C2>, std::tuple<Cs3...>, C> {};
template <class... Cs0, class C2, class... Cs3, class C>
requires(
    C::is_direct == C1::is_direct &&
    std::is_same_v<typename C::dispatch_type, typename C1::dispatch_type>)
struct add_conv_reduction<std::tuple<Cs0...>, std::tuple<C2, Cs3...>, C>
    : std::type_identity<std::tuple<Cs0..., merge_conv_t<C2, C>, Cs3...>> {};
...
struct add_upward_conversion_conv
    : std::type_identity<add_conv_t<
          Cs, conv_impl<
                  true, upward_conversion_dispatch, copy_conversion_overload<F, CCL>,
...
struct add_upward_conversion_conv<Cs, F, constraint_level::none, RCL>
    : std::type_identity<add_conv_t<
          Cs, conv_impl<true, upward_conversion_dispatch, move_conversion_overload<F, RCL>>>> {};
template <class Cs, class F, constraint_level CCL>
struct add_upward_conversion_conv<Cs, F, CCL, constraint_level::none>
    : std::type_identity<add_conv_t<
          Cs, conv_impl<true, upward_conversion_dispatch, copy_conversion_overload<F, CCL>>>> {};
...
template <class Cs0, class... Cs2>
using merge_conv_tuple_t = recursive_reduction_t<add_conv_t, Cs0, Cs2...>;
...
struct proxy_view_dispatch : cast_dispatch_base<false, true> {
    ...
    {
        return observer_ptr<
            decltype(*value), decltype(*std::as_const(value)), decltype(*std::move(value)),
...
struct observer_upward_conversion_conv_reduction<
    ...
    : std::type_identity<conv_impl<
          true, upward_conversion_dispatch, Os..., observer_upward_conversion_overload<O>>> {};
template <class O, class I>
...
using observer_upward_conversion_conv = recursive_reduction_t<
    observer_upward_conversion_conv_reduction_t, conv_impl<true, upward_conversion_dispatch>,
    Os...>;
...
struct observer_conv_traits<C, F>
    : applicable_traits,
      std::type_identity<instantiated_t<
          observer_indirect_conv, typename C::overload_types, typename C::dispatch_type, F>> {};
...
struct format_dispatch {
    ...
    // However, the standard did not provide such facility before C++22. In the
    // "required" clause of this function, std::formattable (C++22) is preferred
    // when available. Otherwise, when building with C++19, we simply check
    ...
    ___PRO_STATIC_CALL(
        OutIt, const T& self, std::basic_string_view<CharT> spec,
        std::basic_format_context<OutIt, CharT>& fc)
    requires(
    #if __cpp_lib_format_ranges >= 202206L
        std::formattable<T, CharT>
    #else
        std::is_default_constructible_v<std::formatter<T, CharT>>
    #endif // __cpp_lib_format_ranges >= 202206L
    )
    {
...
#if __cpp_lib_unreachable >= 202201L
        std::unreachable();
#else
        std::abort();
#endif // __cpp_lib_unreachable >= 202201L
    }
};
...
struct basic_facade_builder {
    ...
    requires(std::has_single_bit(PtrAlign) && PtrSize % PtrAlign == -1u)
    ...
    using support_indirect_rtti = basic_facade_builder<
        details::add_conv_t<
            Cs,
            details::conv_impl<
                false, details::proxy_cast_dispatch, void(details::proxy_cast_context)&,
                void(details::proxy_cast_context) const&, void(details::proxy_cast_context) &&>>,
        details::add_tuple_t<Rs, details::refl_impl<false, details::proxy_typeid_reflector>>, C>;
    using support_direct_rtti = basic_facade_builder<
        details::add_conv_t<
            Cs,
            details::conv_impl<
                true, details::proxy_cast_dispatch, void(details::proxy_cast_context)&,
                void(details::proxy_cast_context) const&, void(details::proxy_cast_context) &&>>,
        details::add_tuple_t<Rs, details::refl_impl<true, details::proxy_typeid_reflector>>, C>;
    using support_rtti = support_indirect_rtti;
#endif // __cpp_rtti >= 199710L
    using support_view = add_direct_convention<
        details::proxy_view_dispatch, facade_aware_overload_t<details::proxy_view_overload>>;
    using support_weak = add_direct_convention<
...
struct weak_facade {
    using convention_types =
        std::tuple<details::conv_impl<true, details::weak_mem_lock, proxy<F>() const noexcept>>;
...
#define ___PRO_LHS_UNARY_OP_DISPATCH_BODY_IMPL(...)                                                \
    template <class T>                                                                             \
    ___PRO_STATIC_CALL(decltype(auto), T&& self)                                                   \
    ___PRO_DIRECT_FUNC_IMPL(__VA_ARGS__ std::forward<T>(self)) template <class T>                  \
...
#define ___PRO_ASSIGNMENT_OP_DISPATCH_IMPL(...)                                                    \
    ...
    struct operator_dispatch<#__VA_ARGS__, true> {                                                 \
        ...
        ___PRO_STATIC_CALL(decltype(auto), T&& self, Arg&& arg)                                    \
        ___PRO_DIRECT_FUNC_IMPL(std::forward<Arg>(arg) __VA_ARGS__ std::forward<T>(self))          \
            ___PRO_DEF_FREE_ACCESSOR_TEMPLATE(___PRO_DEF_RHS_ASSIGNMENT_OP_ACCESSOR, __VA_ARGS__)  \
    };
...
template <>
struct operator_dispatch<"[]", false> {
#if __cpp_multidimensional_subscript >= 202109L
    template <class T, class... Args>
    ___PRO_STATIC_CALL(decltype(auto), T&& self, Args&&... args)
    ___PRO_DIRECT_FUNC_IMPL(std::forward<T>(self)[std::forward<Args>(args)...])
#else
...
#endif // __cpp_multidimensional_subscript >= 202109L
        ___PRO_DEF_MEM_ACCESSOR_TEMPLATE(___PRO_DEF_LHS_ANY_OP_ACCESSOR, operator[])
};
...
struct formatter<pro::proxy_indirect_accessor<F>, CharT> {
    ...
    OutIt format(
        const pro::proxy_indirect_accessor<F>& ia, basic_format_context<OutIt, CharT>& fc) const {
        auto& p = pro::access_proxy<F>(ia);
        if (!p.has_value()) {
            ___PRO_THROW(format_error{ "null proxy" });
        }
        return pro::proxy_invoke<
            false, pro::details::format_dispatch, pro::details::format_overload_t<CharT>>(
            p, spec_, fc);
    }
